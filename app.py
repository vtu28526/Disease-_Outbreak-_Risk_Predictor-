...
def save_prediction(user_id, row, disease_type, score, level, notes):
    with sqlite3.connect(DB_FILE) as conn:
        conn.execute(
            """
            INSERT INTO predictions
            (user_id, fips, state, county, disease_type, risk_score, risk_level, notes, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                user_id,
...
def get_analysis_report():
    ...
    scored_rows.sort(key=lambda item: item["score"], reverse=True)
    history = get_history(session.get("user_id"))
...
def home():
    states = get_states()
    total_counties = len(DATASET)
    latest = get_history(session.get("user_id"))[:3]
...
def predict():
    ...
    if request.method == "POST":
        ...
        if not row:
            ...
            save_prediction(session.get("user_id"), row, disease_type, score, level, notes)
...
def history():
    return render_template(
        "history.html",
        active_page="history",
        predictions=get_history(session.get("user_id")),
    )
